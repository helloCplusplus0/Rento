...
- 任何迁移操作默认都要把旧 `Rento` 源代码视为直接原型；除已在阶段文档中明确批准的最小技术适配外，接近 `100%` 还原旧页面的信息结构、组件表达、导航节奏、表单交互、状态反馈与主链语义，是验收通过的硬门槛之一。
...
- 当执行页面迁移验收时，必须同时核对“新宿主页面是否已经替换占位页”和“新宿主页面是否仍接近 `100%` 还原旧 `Rento` 原型”；只满足前者、不满足后者时，只能判定为中间实现结果，不得判通过。

## 7. 工程校验技能
- 对阶段切换至少检查：文档是否一致、仓库远端是否正确、技术栈是否仍匹配目标方案、验证路径是否可执行。
- 对实现阶段至少检查：`lint`、`type-check`、构建、健康检查与核心 smoke test。
- 对 `phase15` 实现阶段，除通用校验外，还必须额外检查：`manifest.json`、`sw.js`、`/offline`、静态头与 PWA smoke 脚本口径是否与文档一致。
