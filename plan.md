...
- 当前正式云端部署输入已补齐为 GitHub Release 正式部署包链路：`.github/workflows/release-deploy-bundle.yml` 负责生成发布包，服务器通过 `scripts/pull-release-deploy-bundle.sh` 拉取到 `/opt/rento-minix/current`，正式环境文件固定放在 `/etc/rento-minix/rento-minix.env`，且部署过程不执行 build。
