...
- 当前正式云端部署输入已补齐为 GitHub Release 部署包链路：`.github/workflows/release-deploy-bundle.yml` 负责按 `npm run build:minix:pwa` 生成 PWA-enabled 发布包；推送 `main` 会自动生成 `prerelease deploy bundle`，推送 `v*` tag 会自动生成正式 release bundle，`workflow_dispatch` 保留为基于当前选定 ref 的手工正式发包入口；服务器通过 `scripts/pull-release-deploy-bundle.sh` 拉取到 `/opt/rento-minix/current`，正式环境文件固定放在 `/etc/rento-minix/rento-minix.env`，且部署过程不执行 build。
- 当前云端部署操作层继续补齐为：`DEPLOY_RUNBOOK.md` 承接简洁操作步骤，`scripts/prepare-release-host.sh` 承接服务器准备，`scripts/deploy-release-on-server.sh` 承接正式部署执行；`DEPLOYMENT.md` 继续仅承接治理真相源与 rollback-only 边界。
...
- 当前结论：
  ...
  - 已完成 `phase16-02` 自动化验证、`phase16-03` 当前轮源码层对齐复核、cutover 审核包字段冻结、待云端复验占位，以及 `phase16-04` 当前轮任务 `1 ~ 4` 的 legacy-exit 决策与根级真相源同步
  - 五项 legacy 资产已统一冻结为 `rollback-only`：`docker-compose.yml`、`nginx/nginx.conf`、`scripts/cloud-deploy.sh`、`scripts/bootstrap-deploy-assets.sh`、`scripts/start-entry.mjs`
  - 当前轮最终结论固定为 `未通过但单值化`：当前未发现新的源码层 parity 缺口，但正式人工 HTTPS 验收、正式部署演练与 legacy 回滚演练仍待真实云服务器执行
...
- 这组 `phase11` 最低工程验证命令保留为历史阶段门槛；当前 GitHub Release 部署包默认构建 profile 已单值化为 `npm run build:minix:pwa`。
