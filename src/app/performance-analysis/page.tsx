...
export default function PerformanceAnalysisPage() {
  ...
  const [currentNavigation, setCurrentNavigation] = useState<NavigationMetric | null>(null)
  ...
  // 保存数据到localStorage
  const saveToStorage = useCallback((newMetrics: NavigationMetric[], newPerformanceData?: PerformanceData) => {
    try {
  ...
  // 在新窗口中打开页面进行测试
  const testNavigationInNewWindow = useCallback((path: string) => {
  ...
  // 在当前窗口测试（用于对比）
  const testNavigationInCurrentWindow = useCallback((path: string) => {
    const startTime = performance.now()
    const metric: NavigationMetric = {
      id: Date.now().toString(),
      from: pathname,
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Card className="border-blue-200 bg-blue-50">
        ...
        <CardContent className="text-blue-700">
          <div className="space-y-2">
            <p><strong>新窗口测试</strong>：在新窗口打开页面，关闭窗口时记录时间（推荐）</p>
            <p><strong>当前窗口测试</strong>：在当前窗口跳转，返回时自动记录时间</p>
            <p><strong>数据持久化</strong>：所有测试数据会自动保存，页面刷新不会丢失</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            <div>
              <h4 className="font-medium mb-2">新窗口测试（推荐）</h4>
            ...
            <div>
              <h4 className="font-medium mb-2">当前窗口测试</h4>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <Button 
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            {metrics.length === 0 ? (
              <p className="text-gray-500 text-center py-4">暂无导航记录</p>
            ) : (
              metrics.map((metric) => (
                <div 
                  key={metric.id}
                ...
                >
                  <div className="flex items-center gap-3">
                    <Badge className={getStatusColor(metric.status, metric.duration)}>
                      {metric.status}
                    </Badge>
                    <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">
                  {Math.round(performanceData.navigationTiming.domContentLoadedEventEnd - performanceData.navigationTiming.fetchStart)}ms
                </div>
                <div className="text-sm text-gray-500">DOM加载</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">
                  {Math.round(performanceData.navigationTiming.loadEventEnd - performanceData.navigationTiming.fetchStart)}ms
                </div>
                <div className="text-sm text-gray-500">完全加载</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-600">
                  {Math.round(performanceData.navigationTiming.responseStart - performanceData.navigationTiming.fetchStart)}ms
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-600">
                  {Math.round(performanceData.memoryInfo.jsHeapSizeLimit / 1024 / 1024)}MB
                </div>
