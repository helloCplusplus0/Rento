...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!buildingId || typeof buildingId !== 'string') {
      return Response.json({ error: '楼栋ID不能为空' }, { status: 400 })
    }

    if (!Array.isArray(rooms) || rooms.length === 0) {
      return Response.json({ error: '房间数据不能为空' }, { status: 400 })
    }

    // 限制批量创建数量
    if (rooms.length > 100) {
      return Response.json({ error: '单次最多创建100间房间' }, { status: 400 })
    }

    // 验证楼栋存在
    const building = await buildingQueries.findById(buildingId)
    if (!building) {
      return Response.json({ error: '楼栋不存在' }, { status: 404 })
    }
    ...
    for (const roomData of rooms) {
      ...
      if (!roomData.roomNumber || typeof roomData.roomNumber !== 'string') {
        return Response.json({ error: '房间号不能为空' }, { status: 400 })
      }
      ...
      if (isNaN(floorNumber) || floorNumber < 1 || floorNumber > 50) {
        return Response.json({ error: '楼层必须在1-50之间' }, { status: 400 })
      }

      // 验证房间类型
      if (!['SHARED', 'WHOLE', 'SINGLE'].includes(roomData.roomType)) {
        return Response.json({ error: '房间类型无效' }, { status: 400 })
      }
    ...
    return Response.json({ error: '批量创建房间失败，请重试' }, { status: 500 })
  }
}
