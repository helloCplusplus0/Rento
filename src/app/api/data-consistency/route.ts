...
} from '@/lib/data-consistency-checker'
...
export async function GET() {
  try {
    ...
    console.log(
      `[一致性API] 检查完成: ${report.summary.passedChecks}/${report.summary.totalChecks} 通过`
    )
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { issueIds = [], repairOptions = {}, performCheckFirst = true } = body

    console.log(
      `[修复API] 开始数据修复: ${issueIds.length > 0 ? `指定问题 ${issueIds.length} 个` : '所有问题'}`
    )
    ...
    if (performCheckFirst || issueIds.length === 0) {
      ...
      if (issueIds.length > 0) {
        // 修复指定问题
        issuesToRepair = allIssues.filter(
          (issue) => issue.id && issueIds.includes(issue.id)
        )
      } else {
        // 修复所有问题
