...
export default function PerformanceTestPage() {
  ...
  const testVirtualScrolling = useCallback(async () => {
    ...
    return {
      ...
      efficiency:
        Math.round(
          (testData.length / (finalNodeCount - initialNodeCount)) * 100
        ) / 100,
    }
  }, [testData])
  ...
  const testCachePerformance = useCallback(async () => {
    ...
    localStorage.setItem(
      'test-cache-key',
      JSON.stringify(testData.slice(0, 100))
    )
    ...
    const cachedData = JSON.parse(
      localStorage.getItem('test-cache-key') || '[]'
    )
    ...
    return {
      ...
      speedImprovement:
        Math.round((firstRequestTime / secondRequestTime) * 100) / 100,
  ...
  const testMemoryUsage = useCallback(async () => {
    ...
    if (memoryInfo) {
      return {
        usedJSHeapSize:
          Math.round((memoryInfo.usedJSHeapSize / 1024 / 1024) * 100) / 100, // MB
        totalJSHeapSize:
        ...
        <TabsContent value="lazy-load">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-gray-600">滚动查看懒加载效果</p>
            </CardHeader>
            <CardContent>
              <div className="lazy-test-container h-96 overflow-y-auto rounded border p-4">
                  ...
                  {testData.slice(0, 50).map((item, index) => (
                    <div key={item.id} className="text-center">
                      <LazyImage
                        ...
                        showLoadingIndicator
                      />
                      <p className="mt-1 text-xs">图片 {index + 1}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <TabsContent value="api">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-gray-600">测试各API端点的响应时间</p>
            </CardHeader>
            <CardContent>
              {testResults.api && (
              ...
              {Object.keys(testResults).length > 0 ? (
                <div className="space-y-6">
                  ...
                  <div className="rounded bg-gray-50 p-4">
                    <h3 className="mb-2 font-medium">测试总结</h3>
