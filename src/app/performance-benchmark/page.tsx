...
export default function PerformanceBenchmarkPage() {
  ...
  const testJavaScriptPerformance = useCallback(async () => {
    ...
    return {
      ...
      averageTime:
        Math.round(
          (results.reduce((sum, r) => sum + r.time, 0) / results.length) * 1000
        ) / 1000,
    }
  }, [])
  ...
  const testNetworkPerformance = useCallback(async () => {
    ...
    for (const endpoint of endpoints) {
      ...
      if (validMeasurements.length > 0) {
        const avgResponseTime =
          validMeasurements.reduce((sum, m) => sum + m.responseTime, 0) /
          validMeasurements.length
        const avgSize =
          validMeasurements.reduce((sum, m) => sum + m.size, 0) /
          validMeasurements.length
    ...
    return {
      ...
      totalDataTransferred: Math.round(
        results.reduce((sum, r) => sum + r.averageSize, 0) / 1024
      ), // KB
  ...
  const testMemoryUsage = useCallback(async () => {
    ...
    if (memoryInfo) {
      ...
      return {
        ...
        afterCreationMemory:
          Math.round((afterCreationMemory / 1024 / 1024) * 100) / 100,
        afterCleanupMemory:
          Math.round((afterCleanupMemory / 1024 / 1024) * 100) / 100,
        memoryIncrease:
          Math.round(
            ((afterCreationMemory - initialMemory) / 1024 / 1024) * 100
          ) / 100,
        memoryRecovered:
          Math.round(
            ((afterCreationMemory - afterCleanupMemory) / 1024 / 1024) * 100
          ) / 100,
        totalHeapSize:
          Math.round((memoryInfo.totalJSHeapSize / 1024 / 1024) * 100) / 100,
        heapSizeLimit:
          Math.round((memoryInfo.jsHeapSizeLimit / 1024 / 1024) * 100) / 100,
        memoryUsagePercentage: Math.round(
  ...
  const testRenderingPerformance = useCallback(async () => {
    ...
    return {
      results,
      totalRenderTime: results.reduce(
        (sum, r) => sum + (r.renderTime || r.styleTime || 0),
        0
      ),
      totalElements: results.reduce(
        (sum, r) => sum + (r.elementsCreated || r.elementsStyled || 0),
        0
      ),
    }
  }, [])
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {benchmarkResults.pageLoad.domContentLoaded}ms
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>完全加载:</span>
                      <span className="font-medium">
                        {benchmarkResults.pageLoad.loadComplete}ms
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>服务器响应:</span>
                      <span className="font-medium">
                        {benchmarkResults.pageLoad.serverResponseTime}ms
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>网络延迟:</span>
                      <span className="font-medium">
                        {benchmarkResults.pageLoad.networkLatency}ms
                      </span>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    {benchmarkResults.javascript.results.map(
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    {benchmarkResults.network.results.map(
                      (result: any, index: number) => (
                        <div
                          key={index}
                          className="flex justify-between text-sm"
                  ...
                  {benchmarkResults.memory.message ? (
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span>当前内存使用:</span>
                        <span className="font-medium">
                          {benchmarkResults.memory.initialMemory}MB
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>总堆大小:</span>
                        <span className="font-medium">
                          {benchmarkResults.memory.totalHeapSize}MB
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>堆使用率:</span>
                        <span className="font-medium">
                          {benchmarkResults.memory.memoryUsagePercentage}%
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>内存回收:</span>
                        <span className="font-medium">
                          {benchmarkResults.memory.memoryRecovered}MB
                        </span>
                      </div>
                    </div>
                  )}
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  {benchmarkResults.rendering.results.map(
                    (result: any, index: number) => (
