      ...
      <CardContent className="space-y-4">
        ...
        {checkResult && (
          ...
          >
            <div className="mb-2 flex items-center gap-2">
              ...
              <span
                className={`font-medium ${getStatusColor(checkResult.data.isConsistent)}`}
              >
                {checkResult.message}
              </span>
            </div>
            ...
            {!checkResult.data.isConsistent && (
              <div className="space-y-1 text-sm">
                <p>
                  • 孤立记录: {checkResult.data.details.orphanedReadings}个
                  (状态为BILLED但无账单明细)
                </p>
                <p>
                  • 不一致记录: {checkResult.data.details.inconsistentReadings}
                  个 (有账单明细但状态错误)
                </p>
              </div>
            )}
            ...
            {checkResult.data.statistics && (
              <div className="mt-3 border-t border-gray-200 pt-3">
                <div className="text-sm text-gray-600">
                  <p>
                    总抄表记录:{' '}
                    {checkResult.data.statistics.summary.totalReadings}个
                  </p>
                  <p>
                    已生成账单:{' '}
                    {checkResult.data.statistics.summary.totalBilled}个 (
                    {checkResult.data.statistics.summary.billedPercentage}%)
                  </p>
                  <p>
                    最近7天变更:{' '}
                    {checkResult.data.statistics.summary.recentChanges}个
                  </p>
                </div>
              </div>
            )}
        ...
        {repairResult && (
          ...
          >
            ...
            <div className="space-y-1 text-sm">
              <p>
                • 修复前不一致记录: {repairResult.data.preRepairInconsistencies}
                个
              </p>
              <p>
                • 修复后不一致记录:{' '}
                {repairResult.data.postRepairInconsistencies}个
              </p>
              <p>• 孤立记录修复: {repairResult.data.repairedOrphaned}个</p>
              <p>
                • 不一致记录修复: {repairResult.data.repairedInconsistent}个
              </p>

              {repairResult.data.errors.length > 0 && (
                <div className="mt-2">
                  <p className="font-medium text-red-600">修复错误:</p>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-sm text-gray-600">
                          {reading.roomInfo && (
                            <span>
                              {reading.roomInfo.buildingName} -{' '}
                              {reading.roomInfo.roomNumber}
                            </span>
                          )}
                        </div>
                      </div>
