      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-8 text-muted-foreground">
            <p>暂无租客信息</p>
            <p className="text-sm mt-2">房间当前为空房状态</p>
          </div>
        </CardContent>
      </Card>
    )
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        {activeContract.renter && (
          <div>
            <h4 className="font-medium mb-3">租客详情</h4>
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
              ...
              {activeContract.renter.occupation && (
                <div>
                  ...
                  <p className="font-medium">{activeContract.renter.occupation}</p>
                </div>
              )}
              {activeContract.renter.emergencyContact && (
                <div className="col-span-2">
                  <label className="text-sm text-muted-foreground">紧急联系人</label>
                  <p className="font-medium">{activeContract.renter.emergencyContact}</p>
                </div>
              )}
              {activeContract.renter.emergencyPhone && (
                <div className="col-span-2">
                  <label className="text-sm text-muted-foreground">紧急联系电话</label>
                  <p className="font-medium">{activeContract.renter.emergencyPhone}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="pt-4 border-t">
          <h4 className="font-medium mb-3">合同详情</h4>
          <div className="grid grid-cols-2 gap-x-4 gap-y-3">
            <div className="col-span-2">
            ...
            <div>
              <label className="text-sm text-muted-foreground">月租金</label>
              <p className="font-medium text-blue-600">{formatCurrency(activeContract.monthlyRent)}</p>
            </div>
            <div>
              <label className="text-sm text-muted-foreground">押金</label>
              <p className="font-medium">{formatCurrency(activeContract.deposit)}</p>
            </div>
            <div>
              <label className="text-sm text-muted-foreground">合同开始</label>
              <p className="font-medium">{formatDate(activeContract.startDate)}</p>
            </div>
            <div>
              <label className="text-sm text-muted-foreground">合同结束</label>
              <p className="font-medium">{formatDate(activeContract.endDate)}</p>
            </div>
            <div>
              <label className="text-sm text-muted-foreground">总租金</label>
              <p className="font-medium">{formatCurrency(activeContract.totalRent)}</p>
            </div>
            {activeContract.keyDeposit && (
              <div>
                <label className="text-sm text-muted-foreground">门卡押金</label>
                <p className="font-medium">{formatCurrency(activeContract.keyDeposit)}</p>
              </div>
            )}
            {activeContract.cleaningFee && (
              <div>
                <label className="text-sm text-muted-foreground">保洁费</label>
                <p className="font-medium">{formatCurrency(activeContract.cleaningFee)}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {(activeContract.paymentMethod || activeContract.paymentTiming) && (
          <div className="pt-4 border-t">
            <h4 className="font-medium mb-3">付款信息</h4>
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
              ...
              {activeContract.paymentTiming && (
                <div>
                  <label className="text-sm text-muted-foreground">收租时间</label>
        ...
        {(activeContract.signedBy || activeContract.signedDate) && (
          <div className="pt-4 border-t">
            <h4 className="font-medium mb-3">签约信息</h4>
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
              {activeContract.signedBy && (
                <div>
                  <label className="text-sm text-muted-foreground">签约人</label>
                  <p className="font-medium">{activeContract.signedBy}</p>
                </div>
              )}
              {activeContract.signedDate && (
                <div>
                  <label className="text-sm text-muted-foreground">签约时间</label>
                  <p className="font-medium">{formatDate(activeContract.signedDate)}</p>
                </div>
              )}
            </div>
          </div>
