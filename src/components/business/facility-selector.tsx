...
export function FacilitySelector({
  ...
  const [localSelected, setLocalSelected] = useState<string[]>(selectedFacilities)
  ...
  const handleSelectAll = (categoryId: string) => {
    ...
    const otherSelected = localSelected.filter(id => 
      !categoryFacilityIds.includes(id)
    )
    ...
    <div className={cn('space-y-4', className)}>
      ...
      <Card>
        ...
        <CardContent className="pt-0">
          {localSelected.length === 0 ? (
            <p className="text-sm text-muted-foreground">暂未选择任何设施</p>
          ) : (
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className="h-8 px-2 text-xs"
                  disabled={Boolean(maxSelections && localSelected.length >= maxSelections)}
                >
                  全选
                </Button>
              </div>
            ...
            <CardContent className="pt-0">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {category.facilities.map(facility => {
                  ...
                  const isDisabled = !isSelected && Boolean(maxSelections && localSelected.length >= maxSelections)
                  ...
                  return (
                    <FacilityItem
                      ...
                      selected={isSelected}
                      disabled={isDisabled}
                      onClick={() => !isDisabled && handleToggleFacility(facility.id)}
                    />
                  )
                })}
              </div>
...
export const defaultFacilityCategories: FacilityCategory[] = [
  {
    ...
    name: '基础设施',
    facilities: [
      { id: 'wifi', name: 'WiFi', category: 'basic', icon: <FacilityIcons.Wifi /> },
      { id: 'ac', name: '空调', category: 'basic', icon: <FacilityIcons.AirConditioner /> },
  ...
  },
  {
    ...
    name: '家用电器',
    facilities: [
      { id: 'refrigerator', name: '冰箱', category: 'appliances', icon: <FacilityIcons.Refrigerator /> },
      { id: 'washing_machine', name: '洗衣机', category: 'appliances', icon: <FacilityIcons.WashingMachine /> },
      { id: 'microwave', name: '微波炉', category: 'appliances', icon: <FacilityIcons.Microwave /> },
  ...
  },
  {
    ...
    name: '家具配置',
    facilities: [
      { id: 'bed', name: '床', category: 'furniture', icon: <FacilityIcons.Bed /> },
      { id: 'wardrobe', name: '衣柜', category: 'furniture', icon: <FacilityIcons.Wardrobe /> },
      { id: 'desk', name: '书桌', category: 'furniture', icon: <FacilityIcons.Desk /> },
  ...
  },
  {
    ...
    name: '厨房设施',
    facilities: [
      { id: 'stove', name: '燃气灶', category: 'kitchen', icon: <FacilityIcons.Stove /> },
