    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 408) {
          errorMessage = '请求超时，可能是因为数据处理时间较长。请稍后重试，或联系管理员。'
        } else if (response.status === 400) {
          errorMessage = error.details || error.error || '请求参数有误，请检查输入信息'
      ...
      const successMessage = result.data?.message || `合同创建成功${billCount > 0 ? `，已生成${billCount}个账单` : ''}`
      ...
      if (error instanceof Error) {
        if (error.name === 'AbortError') {
          errorMessage = '请求超时，创建合同可能需要较长时间。请稍后重试，或检查网络连接。'
        } else {
