...
function BuildingFilter({ 
  ...
  const buildingOptions = [
    { key: null, label: '全部楼栋', count: Object.values(buildingCounts).reduce((sum, building) => sum + building.count, 0) },
...
export function RoomListPage({ initialRooms }: RoomListPageProps) {
  ...
  const buildingCounts = useMemo(() => {
    ...
    const allBuildingCounts = initialRooms.reduce((acc, room) => {
    ...
    if (selectedStatus || searchQuery) {
      ...
      const filteredBuildingCounts = filteredByOtherConditions.reduce((acc, room) => {
        const buildingId = room.building.id
        if (!acc[buildingId]) {
          acc[buildingId] = {
            name: room.building.name,
      ...
      Object.keys(allBuildingCounts).forEach(buildingId => {
        if (filteredBuildingCounts[buildingId]) {
          allBuildingCounts[buildingId].count = filteredBuildingCounts[buildingId].count
        } else {
      ...
      <div className="space-y-6 pb-6">
        ...
        <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100">
          <div className="space-y-3">
            <h3 className="text-sm font-medium text-gray-700">按房间状态筛选</h3>
            <RoomStatusFilter
