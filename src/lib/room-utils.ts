...
export function parseRoomQueryParams(searchParams: URLSearchParams) {
  return {
    ...
    filters: {
      buildingIds:
        searchParams.get('buildingIds')?.split(',').filter(Boolean) ||
        undefined,
      floorNumbers:
        searchParams
          .get('floorNumbers')
      ...
      statuses:
        (searchParams
          .get('statuses')
          ?.split(',')
          .filter(Boolean) as RoomStatus[]) || undefined,
      roomTypes:
        (searchParams.get('roomTypes')?.split(',').filter(Boolean) as (
          | 'SHARED'
          | 'WHOLE'
          | 'SINGLE'
        )[]) || undefined,
      rentRange:
        (searchParams
          ...
          ?.split(',')
          .map(Number)
          .filter((n) => !isNaN(n)) as [number, number]) || undefined,
      areaRange:
        (searchParams
          .get('areaRange')
          ?.split(',')
