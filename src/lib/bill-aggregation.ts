...
export function groupReadingsByContract(readings: any[]): Map<string, MeterReadingData[]> {
  ...
  for (const reading of readings) {
    ...
    if (readingData.amount === 0 && readingData.usage > 0 && readingData.unitPrice > 0) {
      ...
      console.log(`[聚合] 重新计算金额: ${readingData.meterName} ${readingData.usage} × ${readingData.unitPrice} = ${readingData.amount}`)
    }
  ...
  for (const [contractId, contractReadings] of grouped) {
    console.log(`[聚合] - 合同 ${contractId}: ${contractReadings.length} 个仪表`)
  }
 ...
 * 生成聚合账单备注
 */
function generateAggregatedRemarks(readingDataList: MeterReadingData[]): string {
  const remarkParts = []
  ...
  for (const data of readingDataList) {
    ...
    remarkParts.push(`${meterTypeName}${data.amount}元(${data.usage}${data.unit})`)
  }
