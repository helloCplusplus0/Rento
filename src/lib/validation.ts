...
import {
  performContractDeleteSafetyCheck as performContractDeleteSafetyCheckFromDomain,
  performRoomDeleteSafetyCheck,
} from './domain/delete-guards'
 ...
 *
 * compat wrapper:
 * phase09-02 起正式门禁规则已迁入 src/lib/domain/delete-guards，
 * 当前函数仅保留历史导出名，避免旧调用点继续持有第二套实现。
 */
export async function performDeleteSafetyCheck(roomId: string) {
  return performRoomDeleteSafetyCheck(roomId)
}
...
export async function performContractDeleteSafetyCheck(contractId: string) {
  return performContractDeleteSafetyCheckFromDomain(contractId)
}
