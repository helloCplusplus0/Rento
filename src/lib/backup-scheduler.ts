...
export class BackupScheduler {
  ...
  private scheduleBackup(name: string, config: ScheduleConfig): void {
    ...
    console.log(`📅 备份调度已设置: ${name} (下次执行: ${nextExecution.toLocaleString()})`)
  }

  /**
   * 执行调度的备份任务
   */
  private async executeScheduledBackup(name: string, config: ScheduleConfig): Promise<void> {
    try {
      ...
      // 清理旧备份
      const deletedCount = await backupManager.cleanupOldBackups(config.retentionDays)
   ...
   * 发送备份告警
   */
  private async sendBackupAlert(backupName: string, error: unknown): Promise<void> {
