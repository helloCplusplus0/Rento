 ...
 * 用于全面的数据分析和报表生成
 */
export async function getAllMeterHistoryStats(
  options: {
  ...
  const stats: MeterHistoryStats[] = meters.map((meter) => {
    ...
    const firstReadingDate =
      readings.length > 0 ? readings[0].readingDate : null
    const lastReadingDate =
      readings.length > 0 ? readings[readings.length - 1].readingDate : null
    ...
    if (firstReadingDate && lastReadingDate && totalUsage > 0) {
      const monthsDiff = Math.max(
        1,
        (lastReadingDate.getTime() - firstReadingDate.getTime()) /
          (1000 * 60 * 60 * 24 * 30)
      )
 ...
 * 专门用于分析已移除仪表的历史贡献
 */
export async function getRemovedMeterStats(
  options: {
    startDate?: Date
    endDate?: Date
    meterType?: MeterType
 ...
 * 包括已移除仪表的数据，确保统计完整性
 */
export async function getMeterTypeHistorySummary(
  options: {
    startDate?: Date
    endDate?: Date
...
> {
  ...
  meterTypes.forEach((type) => {
    ...
    if (typeStats.totalMeters > 0) {
      typeStats.averageUsagePerMeter =
        Math.round((typeStats.totalUsage / typeStats.totalMeters) * 100) / 100
    }
  })
...
export async function validateMeterDataIntegrity(meterId: string): Promise<{
  ...
  try {
    ...
    if (readingsCount === 0) {
      ...
      if (!meter.isActive) {
        recommendations.push(
          `该仪表已被软删除，${readingsCount}条历史记录已保护`
        )
      } else {
        recommendations.push(
          `该仪表有${readingsCount}条历史记录，建议使用软删除`
        )
      }
    }
    ...
    if (billsCount > 0) {
      recommendations.push(
        `该仪表关联${billsCount}个账单，历史数据对财务分析很重要`
      )
    }
 ...
 * 包括已移除仪表的贡献分析
 */
export async function generateMeterHistoryReport(
  options: {
    startDate?: Date
    endDate?: Date
  ...
  const report = {
    ...
    dataIntegrity: {
      ...
      removedMeterContribution: {
        ...
        percentage:
          allStats.length > 0
            ? Math.round((removedStats.length / allStats.length) * 100 * 100) /
              100
            : 0,
      },
    },
