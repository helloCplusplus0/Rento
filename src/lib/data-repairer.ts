...
export class DataRepairer {
  ...
  async repairDataInconsistencies(
    ...
    for (const issue of issuesToRepair) {
      try {
        ...
        if (repairResult.success) {
          ...
          result.skippedIssues++
          console.log(`[数据修复] 跳过修复: ${issue.description} - ${repairResult.message}`)
        }
    ...
    console.log(`[数据修复] 修复完成: 成功 ${result.repairedIssues}, 跳过 ${result.skippedIssues}, 失败 ${result.failedIssues} (耗时: ${result.executionTime}ms)`)
      ...
      async (tx) => {
        ...
        switch (issue.suggestedFix) {
          ...
          case 'updateRoomStatus':
            return await this.updateRoomStatus(tx, issue.entityId, issue.metadata)
   ...
   * 更新房间状态
   */
  private async updateRoomStatus(tx: any, entityId: string, metadata?: Record<string, any>): Promise<void> {
   ...
   * 重新计算明细金额
   */
  private async recalculateDetailAmounts(tx: any, detailId: string): Promise<void> {
