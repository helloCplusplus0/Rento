...
export async function validateReadingBillConsistency(): Promise<ValidationResult> {
  ...
  try {
    ...
    console.log(`[一致性检查] 完成 - 孤立记录: ${orphanedReadings.length}个, 不一致记录: ${inconsistentReadings.length}个`)
...
export async function repairReadingStatusInconsistencies(): Promise<RepairResult> {
  ...
  for (const reading of validation.orphanedReadings) {
    try {
      ...
      console.log(`[状态修复] 重置孤立记录状态: ${reading.id} (${reading.meter.displayName})`)
    } catch (error) {
  ...
  for (const reading of validation.inconsistentReadings) {
    try {
      ...
      console.log(`[状态修复] 更新不一致记录状态: ${reading.id} (${reading.meter.displayName})`)
    } catch (error) {
  ...
  console.log(`[状态修复] 完成 - 孤立记录修复: ${repairResults.repairedOrphaned}个, 不一致记录修复: ${repairResults.repairedInconsistent}个, 错误: ${repairResults.errors.length}个`)
  ...
  try {
    ...
    const result = {
      ...
      summary: {
        totalReadings,
        totalBilled,
        billedPercentage: totalReadings > 0 ? Math.round((totalBilled / totalReadings) * 100) : 0,
        recentChanges
      }
    }
    
    console.log(`[状态统计] 完成 - 总记录: ${totalReadings}个, 已生成账单: ${totalBilled}个 (${result.summary.billedPercentage}%)`)
...
export async function checkSingleReadingConsistency(readingId: string) {
  ...
  try {
    ...
    console.log(`[单记录检查] 完成 - ${reading.meter.displayName}: ${result.isConsistent ? '一致' : '不一致'} (${result.issues.length}个问题)`)
...
export async function batchCheckReadingConsistency(readingIds: string[]) {
  ...
  console.log(`[批量检查] 完成 - 一致: ${summary.consistent}个, 不一致: ${summary.inconsistent}个, 错误: ${summary.errors}个`)
