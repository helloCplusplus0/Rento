...
export async function repairAllUtilityBillDetails(): Promise<RepairResult> {
  ...
  try {
    ...
    console.log(`[修复工具] 找到 ${billsWithoutDetails.length} 个需要修复的账单`)
    ...
    for (const bill of billsWithoutDetails) {
      try {
        ...
        if (repaired) {
          ...
          result.skippedCount++
          console.log(`[修复工具] 跳过账单: ${bill.billNumber} (无法找到相关抄表记录)`)
        }
      } catch (error) {
    ...
    console.log(`[修复工具] 修复完成: 成功 ${result.repairedCount} 个, 跳过 ${result.skippedCount} 个, 错误 ${result.errors.length} 个`)
...
export async function repairSingleBillDetails(bill: any): Promise<boolean> {
  ...
  if (bill.metadata) {
    try {
      const metadata = typeof bill.metadata === 'string' ? JSON.parse(bill.metadata) : bill.metadata
      if (metadata.breakdown && Array.isArray(metadata.breakdown)) {
        detailsFromMetadata = metadata.breakdown
        console.log(`[修复工具] 从metadata中找到 ${detailsFromMetadata.length} 条明细信息`)
      }
    } catch (error) {
      console.warn(`[修复工具] 解析账单 ${bill.billNumber} 的metadata失败:`, error)
    }
  }
  ...
  if (detailsFromMetadata.length > 0 && detailsFromReadings.length > 0) {
    ...
    detailsToCreate = detailsFromMetadata.map((metaDetail: any) => {
      const matchingReading = detailsFromReadings.find(reading => 
  ...
  if (detailsToCreate.length > 0) {
    ...
    console.log(`[修复工具] 为账单 ${bill.billNumber} 创建了 ${detailsToCreate.length} 条明细记录`)
    return true
  }
