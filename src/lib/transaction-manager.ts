...
export class TransactionManager {
  ...
  async executeTransaction<T>(
    ...
    while (retryCount <= finalOptions.retryAttempts) {
      try {
        console.log(`[事务管理器] 开始执行事务: ${transactionContext.description} (尝试 ${retryCount + 1}/${finalOptions.retryAttempts + 1})`)
        ...
        console.log(`[事务管理器] 事务执行成功: ${transactionContext.description} (耗时: ${executionTime}ms)`)
        ...
        const shouldRetry = this.shouldRetryError(errorType, retryCount, finalOptions.retryAttempts)
