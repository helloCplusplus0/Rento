...
## 1. 文档职责
- 本文档只负责环境变量与配置规则
- 快速启动命令请看 [QUICK_START.md](file:///home/dell/Projects/Rento/QUICK_START.md)
- 私有部署、PWA 与验收步骤请看 [DEPLOYMENT.md](file:///home/dell/Projects/Rento/DEPLOYMENT.md)
...
- `LOCAL_HTTPS_PORT`：本地私有 HTTPS 验收的高位端口，默认 `18443`
- `APP_INTERNAL_PORT`：容器内应用监听端口
- `APP_PORT`：宿主机暴露端口；默认与 `APP_INTERNAL_PORT` 保持一致
- `APP_BIND_ADDRESS`：应用容器映射到宿主机时的监听地址；默认 `0.0.0.0`
...
- `APP_IMAGE`：容器部署时使用的应用镜像名；默认指向 GHCR，可保留镜像部署路径
...
- `POSTGRES_BIND_ADDRESS`：PostgreSQL 对宿主机暴露的绑定地址；默认 `127.0.0.1`
...
- `REDIS_BIND_ADDRESS`：Redis 对宿主机暴露的绑定地址；默认 `127.0.0.1`
- `LOG_DIR` / `BACKUP_DIR`：宿主机可写路径
- `HOST_LOG_DIR` / `HOST_BACKUP_DIR` / `HOST_NGINX_LOG_DIR`：容器挂载到宿主机的真实路径
- `NGINX_SSL_DIR`：容器化 Nginx 使用的证书宿主机路径
...
- 部署命令、PWA 验收与回退步骤统一以 [DEPLOYMENT.md](file:///home/dell/Projects/Rento/DEPLOYMENT.md) 为准
