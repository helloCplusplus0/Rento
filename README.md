...
- 当前 GitHub 发布链已切换为：`.github/workflows/release-deploy-bundle.yml` 负责执行 `npm run build:minix`、校验 `dist/` 与 `build/minix-server/`，并把部署包上传为 GitHub Release asset；推送 `main` 会自动生成 `prerelease deploy bundle`，推送 `v*` tag 会自动生成正式 release bundle，`workflow_dispatch` 保留为基于当前选定 ref 的手工正式发包入口。
...
- `main` 自动生成的 `prerelease deploy bundle` 用于快速验证、预演练与待验收环境，不改写“正式部署默认使用 `v*` 正式 release bundle”的口径。
...
- 当前正式部署包默认包含：`dist/`、`build/minix-server/`、运行时 `node_modules/`、`prisma/`、`deploy/`、`.env.example`、`package.json`、`package-lock.json` 与正式启动/健康检查/迁移/拉包脚本。
