...
- 真实云服务器上的正式部署与人工实测已由 [ECS_Deployment_Experience_1.md](file:///home/dell/Projects/Rento/ECS_Deployment_Experience_1.md) 形成证据闭环：`Rento-miniX` 已可独立交付与继续推进；legacy 回滚演练不再作为本轮通过前置条件。
...
- 当前 legacy 参考资产仍是旧 `docker-compose + nginx` 容器化运行线，其资产清单固定为 `docker-compose.yml`、`nginx/nginx.conf`、`scripts/cloud-deploy.sh`、`scripts/bootstrap-deploy-assets.sh`、`scripts/start-entry.mjs` 以及历史镜像、容器、`nginx`、`redis` 变量口径；其中历史 `Next.js standalone` 直接启动入口继续由 `scripts/start-entry.mjs` 保留，用于只读对照与未来归档前审计。
- legacy 资产只承担历史运行参考、差异对照与只读参考职责，不再作为默认部署入口、默认运维入口、正式真相源或正式回退依赖。
- 当前正式回退口径已切换为 `Rento-miniX` 自身的 release 级回退：优先回切到上一个可用 release bundle，并结合环境文件/数据库恢复与健康检查复验完成处置；legacy GHCR 镜像、legacy deployment artifact 与 `scripts/bootstrap-deploy-assets.sh` 都不得再被描述为正式入口。
- legacy 资产将在未来独立治理任务中决定是否归档或彻底移除；当前轮继续保留其参考价值，不直接删除。
...
- 当前已完成 `phase16` 当前轮收口：`phase16-01` 已完成证据入口收口、四类 parity matrix 回填与根级真相源一致性复核，`phase16-02` 已完成固定自动化验证，`phase16-03` 已完成源码层对齐复核与 cutover 审核包字段冻结，`phase16-04` 已把旧 `Rento` 资产统一降级为只读参考 / 差异对照资产；后续默认进入 `Rento-miniX` 的独立 fix 与增量迭代。
- 当前默认路线图继续固定为以下阶段顺序：
...
- 原容器化部署链仍作为当前存量运行线的只读参考与差异对照保留，保留目的仅限历史实现参考、运维经验追溯与未来归档前审计，不自动等同于 `Rento-miniX` 的正式部署主线。
- legacy 资产未来只有在明确完成参考内容吸收、差异审计与替代引用复核后，才允许进入后续退出决策。
