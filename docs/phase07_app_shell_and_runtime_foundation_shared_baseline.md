...
### 9.1 旧 `src/app` 保留边界
- 旧 `src/app` 在 `phase07` 结束后继续承担：
  - 现状页面壳参考基线
  - 未迁移页面的存量运行职责
  - UI 行为、信息架构和路由对照输入
- 旧 `src/app` 在 `phase07` 结束后不再承担：
  - 新前端壳默认宿主
  - 新路由骨架默认落点
  - 新增页面壳长期承接位

### 9.2 旧 `src/app/api/*` 保留边界
- 旧 `src/app/api/*` 在 `phase08` 前继续承担：
  - 存量业务 API
  - 存量认证 API
  - 兼容宿主和参考契约职责
- 旧 `src/app/api/*` 在 `phase08` 起不再默认承担：
  - 新 API 宿主
  - 新认证中间件骨架
  - 新错误处理与请求约束的长期承接位

### 9.3 旧启动脚本保留边界
- `scripts/dev-entry.mjs` 继续服务于旧 `Next.js` 开发态和现状对照验证
- `scripts/start-entry.mjs` 继续服务于旧 `Next.js` 存量启动和回滚基线
- 在新主线脚本与回滚说明未冻结前，不允许把旧脚本视为可直接删除资产

### 9.4 并行关系
- 开发态并行：`src/minix/` + `server/` 承接新主线验证；旧 `Next.js` 运行线保留为现状行为对照
- 验证态并行：新宿主验证新壳、路由、中间件和 `/api/health`；旧宿主验证未迁移入口和回滚可行性
- 存量运行线并行：在部署主线切换前，旧宿主继续承担当前可运行宿主职责；新宿主只冻结正式承接方向
...
### 9.6 `phase08` / `phase09` 直接输入
- `phase08-api-and-auth-foundation`
  - 正式宿主输入：`server/`、基础中间件、最小环境变量读取层、`/api/health` 承接位
  - 存量参考输入：旧 `src/app/api/*`、`src/middleware.ts`、`src/lib/auth/*`、`src/lib/api-error-handler.ts`
  - 禁止越界：不提前迁移正式领域服务、不切部署主线、不删除旧宿主代码
- `phase09-domain-service-migration`
  - 正式宿主输入：`server/` 的统一 API 宿主承接位、`src/minix/` 的前端壳与路由承接位
  - 存量参考输入：旧 `src/app/*`、旧 `src/app/api/*`、`src/lib/prisma.ts`、`src/lib/queries.ts`
  - 禁止越界：不把数据访问层定案、部署切线或 UI 重设计混入领域迁移
...
- 至少确认：
  ...
  - 旧宿主、新宿主与后续阶段的关系已明确
  - 旧前端壳、旧 API 宿主与旧启动脚本的退出条件已可执行
  - `phase08` 与 `phase09` 的直接输入清单已明确
