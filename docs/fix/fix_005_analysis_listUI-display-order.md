...
- 第三类：账单默认展示在业务上更接近“处理队列”，应先看到最紧急、最早该处理的单子；当前代码却把更晚到期的账单排在前面，和队列语义相反。
...
- 根因三：系统虽然已经有统一的账单展示状态语义 `OPEN / OVERDUE / SETTLED`，但这套语义目前主要用于统计和筛选，不用于列表排序；因此“未完结账单前置、组内最早到期优先”的队列语义只存在于业务期望中，没有被实现成统一排序规则。
...
- 服务端实际创建链路：
  ...
  - 账单列表页查询 [billQueries.findAll](file:///home/dell/Projects/Rento/src/lib/queries.ts#L914-L927) 在修正前明确按 `orderBy: { dueDate: 'desc' }` 返回，这会把到期更晚的账单排到更前面。
  - `/api/bills` 对应的优化查询 [optimized-queries.ts](file:///home/dell/Projects/Rento/src/lib/optimized-queries.ts#L710-L750) 在修正前也采用 `dueDate desc + createdAt desc`，与“最早到期优先”的处理队列语义相反。
- 数据模型映射：
...
- 历史已生成账单：
  - 影响已经存在的全部账单集合，尤其是在“待处理账单中，最早到期单据反而排在更后面”的场景下最明显。
- 统计 / 仪表盘：
...
- 做法：
  ...
  - 在该工具中冻结统一规则：
    - 第一层：未完结账单优先于已完结账单
    - 第二层：在同一结算阶段内按 `dueDate asc` 排序，越早到期越靠前
...
- 风险：
  ...
  - 需要先把“列表更像处理队列”这一业务语义冻结清楚，否则排序规则仍可能反复在“回顾视角”与“队列视角”之间摇摆。
...
- 推荐原因：
  ...
  - 经真实样例验证，账单默认列表在业务上更接近“处理队列”，因此排序不应采用“更晚到期排前”，而应采用“更早到期排前”。
- 实施边界：
  - 先冻结排序语义：
    - 未完结账单（如 `PENDING / OVERDUE / 部分支付仍有待收余额`）优先展示
    - 已完结账单（如 `PAID / COMPLETED / pendingAmount≈0`）后置展示
    - 在各自分组内按 `dueDate asc` 排列，越早到期越靠前
  - 时间字段建议优先使用：
    - `dueDate` 作为业务紧迫度主键
    - `createdAt` 作为同日稳定兜底
  - 页面范围优先收口：
...
- 未完结账单必须稳定排在已完结账单之前。
- 在同一展示分组内，账单必须按 `dueDate asc` 展示，越早到期越靠前。
