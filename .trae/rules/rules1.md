...
- 当前默认优先完成 `phase07-app-shell-and-runtime-foundation` 的阶段级规划与审核，不直接扩张新功能面。
...
- 当前默认工作流已从旧 `fix` 闭环切换到 `phase07-app-shell-and-runtime-foundation` 的规划阶段；正式实现必须建立在 `phase07` 阶段文档审核通过之后。
- `phase06` 审核通过的最低前提，不仅包括根级真相源、目录治理和仓库状态收口，还包括：完整 `Hono` 路线图、模块分类与文件级吸收映射已冻结并通过审核。
- `phase07` 审核通过的最低前提，至少包括：前端路由方案、开发拓扑、并行壳切入策略、实现目录、脚本方案与最小环境变量口径均已冻结并通过审核。
...
- `phase07` 期间允许建立 `Hono + Vite` 的新应用壳与运行时承接位，但不得在该阶段直接迁移合同、账单、仪表、抄表等主链领域服务，也不得直接切换最终部署主线。
