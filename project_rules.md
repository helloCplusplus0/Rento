...
- 正式服务器环境文件固定放置在 `/etc/rento-minix/rento-minix.env`；正式应用目录固定为 `/opt/rento-minix/current`。
...
- `phase11` 之后若调整 GitHub 正式部署包、Release asset 命名、服务器拉包脚本、`systemd`/`Caddy` 刷新步骤或健康检查步骤，必须同步更新 `.github/workflows/*`、`scripts/pull-release-deploy-bundle.sh`、`DEPLOYMENT.md` 与根级真相源。
...
- `phase15` 期间允许把旧 Next PWA 宿主中的 `manifest`、`service worker`、安装提示、更新提示与最小离线兜底迁入 `src/minix/*`、根级 `public/*`、`index.html`、`.env.example`、`scripts/pwa-smoke-check.sh` 与 `server/lib/static.ts` 的单一交付链路，但不得在该阶段反向重开正式业务 API 迁移、页面 parity 或离线数据库/离线写入议题。
- `phase15` 当前轮最低文档验证要求固定为：`docs/phase15_*` 互链复核、被引用 `src/minix/*`、`src/components/pwa/*`、`public/*`、`server/lib/static.ts`、`scripts/pwa-smoke-check.sh` 与 `index.html` 路径存在性复核，以及 `README.md`、`AGENTS.md`、`project_rules.md`、`architecture_map.md`、`plan.md` 与 `docs/phase14_*`、`docs/phase15_*` 状态一致性复核；进入实施后最低工程验证要求固定为：`npm run lint`、`npm run type-check`、`npm run build:minix`，并补 `bash ./scripts/pwa-smoke-check.sh --base-url <runtime-url>` 或等价 PWA smoke。
- `phase16` 期间允许把页面/API/PWA/部署四类 parity matrix、自动化验证结果、人工浏览器验收、cutover 审核、回滚演练与 legacy 退出顺序收口到 `docs/phase16_*`、根级真相源与 `DEPLOYMENT.md` 的单一解释上，但固定落位必须保持为：四类 parity matrix 回写 `docs/phase16_parity_verification_cutover_and_legacy_exit_shared_baseline.md`，自动化验证、人工验收、cutover 审核、部署/回滚演练与 legacy 退出判断回写 `docs/phase16_parity_verification_cutover_and_legacy_exit_dev_plan.md`；不得在该阶段反向重开正式业务 API 迁移、页面重设计、PWA 方案重做或 ORM 主线替换。
- `phase16` 当前轮最低文档验证要求固定为：`docs/phase16_*` 互链复核、被引用 `src/minix/*`、`src/app/**/page.tsx`、`src/app/api/*`、`server/lib/legacy-route-inventory.ts`、`scripts/phase09-05-main-flow-smoke.ts`、`scripts/phase14-06-query-cutover-smoke.ts`、`scripts/pwa-smoke-check.sh`、`scripts/health-check.sh`、`deploy/caddy/Caddyfile`、`deploy/systemd/rento-minix.service`、`docker-compose.yml` 与 `nginx/nginx.conf` 路径存在性复核，以及 `README.md`、`AGENTS.md`、`project_rules.md`、`architecture_map.md`、`plan.md`、`DEPLOYMENT.md` 与 `docs/phase13_*`、`docs/phase14_*`、`docs/phase15_*`、`docs/phase16_*` 状态一致性复核；进入实施后最低工程验证要求固定为：`npm run lint`、`npm run type-check`、`npm run build:minix`、`npm run audit:phase09:legacy-routes`、`npm run smoke:phase09:all`、`npm run smoke:phase14:wave2`、`npm run build:minix:pwa`、`bash ./scripts/pwa-smoke-check.sh --profile production-ready --base-url <runtime-url>` 与 `bash ./scripts/health-check.sh --url <runtime-url>`。
...
- `phase16` 当前轮虽已把五项 legacy 资产职责、保留边界、退出前提与回滚窗口单值化，但由于正式人工 HTTPS 验收、正式部署演练与 legacy 回滚演练尚未在真实云服务器执行，当前最终结论仍必须保持为 `未通过但单值化`，不得提前写成 `通过`。
